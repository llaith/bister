...
public interface TaskService {
    ...
    TaskQueueInfo getQueueInfo(String sessionUid, String watermark, String queue);

    List<TaskQueueInfo> listQueueInfo(String sessionUid, String watermark, int limit);

    List<Lock> listQueueLocks(String sessionUid, Date lastDate, Integer lastId, int limit);

    String lockNextQueue(String sessionUid, String watermark);

    String lockRequestedQueue(String sessionUid, String watermark, String queue);

    void releaseQueueLock(String sessionUid, String lockUid);

    String submitTask(String sessionUid, String lockUid, TaskSubmission init); // returns a handle for tracking, not the task because that would bypass the locking.

    TaskStatus taskStatus(String sessionUid, String taskUid); // returns the tracking info.

    Task nextTask(String sessionUid, String lockUid); // picks up the locked task

    Task updateTask(String sessionUid, String lockUid, TaskUpdate update); // POST

    void deleteTask(String sessionUid, String lockUid, String taskUid, String reason); // POST. lock keeps this safe

    int deleteQueue(String sessionUid, String lockUid, String reason); // POST. lock keeps this safe

    int cleanQueue(String sessionUid, String lockUid, Date before); // can send now() to delete all.
